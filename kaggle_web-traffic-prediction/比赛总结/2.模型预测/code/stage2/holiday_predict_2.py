...
def holiday_model_log(df_train, lang, review=False):
    ...
    pre_df['ds'] = pd.date_range('9/13/2017', '11/13/2017')
	...
	future = pd.DataFrame(columns=['ds'])
        time_series = pd.date_range('9/13/2017', '11/13/2017')
        future['ds'] = time_series
        forecast = m.predict(future)
        #print forecast['yhat']

        pre_y = np.array(forecast['yhat'])
...
if __name__ == '__main__':
    start = time.time()
    # get page_date --> key
    key_df = pd.read_csv('data/key_2.csv')
    # get train_data
    ...
    output_df['Id'] = key_df['Id'][62 * pre_min:62*pre_max]

    pages_id = []
    for i in range(pre_min, pre_max):
        page = re.sub('_\d+-\d+-\d+$', '', key_df['Page'][62 * i])
    ...
    for index, page_row_id in enumerate(pages_id):
        ...
        output_df['Visits'][62 * index:62 * (index + 1)] = np.array(prediction['yhat'])
    output_df.to_csv('sub/submission'+str(pre_max)+'.csv', index=False)
